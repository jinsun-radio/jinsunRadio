...
// 志工走到長輩家附近（預設 250 公尺）就先播「志工已經到了！請待在原地不要移動」。
// 為什麼提早幾十秒：讓長輩知道來的是我們派的人、心裡有底；但**絕不叫長輩起身去開門**
// ——獨居長輩（尤其剛疑似跌倒）不該為了應門而移動，由志工進門查看最安全。
//
...
export function speakText(status, { name, eta, kind } = {}) {
  ...
  // ③ 到場（approaching 由 GPS 預告、arrived 由狀態，兩者擇一播一次即可）。
  // 一律不叫長輩「開門／到門口」——獨居長輩（尤其剛疑似跌倒）不該被要求起身去應門，
  // 由志工進門查看。緊急單直接叫長輩「待在原地不要移動」，避免亂動加重傷勢。
  if (status === 'arrived' || status === 'approaching') {
    if (kind === 'supply') {
      return `${who}到了，東西幫您送進來，您不用起身，安心休息就好。`;
    }
    return `${who}已經到了！請待在原地不要移動，馬上進來看您。`;
  }
